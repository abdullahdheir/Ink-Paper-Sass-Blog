                        ...
                        <div class="space-y-2 max-h-[200px] overflow-y-auto">
                            ...
                            @empty
                                <p class="text-secondary font-metadata text-metadata">No tags available. <a
                                        href="{{ route('manage.tags.create') }}" class="text-primary hover:underline">Create
                                        one</a></p>
                            @endforelse
                        </div>
                    </section>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateTags() {
                ...
                tagsArray.forEach((tag, index) => {
                    ...
                    tagItem.className =
                        'bg-primary-fixed text-on-primary-fixed px-3 py-1 rounded-full font-metadata text-metadata flex items-center gap-1';
                    tagItem.innerHTML =
                        `${tag} <span class="material-symbols-outlined text-[14px] cursor-pointer remove-tag" data-index="${index}">close</span>`;
                    tagsContainer.appendChild(tagItem);
                });
                // حفظ التاجز بـ Hidden input كـ JSON string عشان يستلمها لارافيل بـ $request->tags
                hiddenTagsInput.value = JSON.stringify(tagsArray);
            }
                    ...
                    .then(response => {
                        ...
                        if (tags.length) {
                            tags.forEach(t => {
                                ...
                                tagOption.className =
                                    'px-4 py-2 hover:bg-surface-container-high cursor-pointer';
                                tagOption.textContent = t.name;
                                tagOption.addEventListener('click', () => {
                                    if (!tagsArray.includes(t.id)) {
                                        tagsArray.push(t.id);
                                        updateTags();
                                    }
                                    tagSearchContainer.innerHTML = '';
                                ...
                                tagSearchContainer.appendChild(tagOption);
                            });
                        } else {
                            const noResult = document.createElement('div');
                            noResult.className = 'px-4 py-2 text-secondary';
                            noResult.textContent = 'No tags found';
                            tagSearchContainer.appendChild(noResult);
                        }
                    })
                    .catch(error => {
