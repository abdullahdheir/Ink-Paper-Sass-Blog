...
<div class="space-y-6">
    ...
    <div class="bg-surface border border-outline-variant rounded-2xl p-6">
        ...
        <p class="font-body-md text-body-md text-secondary">Control how others see you on the Ink &amp; Paper platform.</p>
    </div>
    ...
    <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
        ...
        @if (session('status') === 'profile-updated')
            <div class="mb-4 flex items-center gap-3 bg-primary-container/10 border border-primary/20 text-primary p-4 rounded-xl font-ui-label text-ui-label">
        ...
        @if ($errors->any())
            <div class="mb-4 bg-error-container/20 border border-error/20 text-error p-4 rounded-xl font-ui-label text-ui-label">
        ...
        <div class="bg-surface border border-outline-variant rounded-2xl p-6 mb-6">
            ...
            <div class="flex items-center gap-6">
                <div class="relative group flex-shrink-0">
                    <div class="w-24 h-24 rounded-full overflow-hidden border-2 border-outline-variant bg-surface-container-low">
        ...
        <div class="bg-surface border border-outline-variant rounded-2xl p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="flex flex-col gap-1.5">
                    <label class="font-ui-label text-ui-label text-on-surface-variant font-medium" for="name">Full Name</label>
                    ...
                    @error('name') <p class="text-error font-metadata text-metadata">{{ $message }}</p> @enderror
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="font-ui-label text-ui-label text-on-surface-variant font-medium" for="username">Username</label>
                    <div class="relative">
                        <span class="absolute left-4 top-1/2 -translate-y-1/2 text-secondary font-body-md select-none">@</span>
                    ...
                    @error('username') <p class="text-error font-metadata text-metadata">{{ $message }}</p> @enderror
                </div>
            </div>
        </div>
        ...
        <div class="bg-surface border border-outline-variant rounded-2xl p-6 mb-6">
            ...
            <div class="flex flex-col gap-1.5">
                <textarea id="bio" name="bio" rows="4"
                    ...
                    maxlength="500"
                    placeholder="Tell the world a little bit about yourself...">{{ old('bio', auth()->user()->bio) }}</textarea>
        ...
        <div class="bg-surface border border-outline-variant rounded-2xl p-6 mb-6">
            ...
            <div class="space-y-4">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 flex items-center justify-center border border-outline-variant rounded-xl bg-surface-container-low flex-shrink-0">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
            <div class="p-5 bg-surface-container rounded-xl border border-outline-variant flex gap-4">
                <span class="material-symbols-outlined text-primary mt-0.5 flex-shrink-0" style="font-variation-settings: 'FILL' 1;">visibility</span>
                <div>
                    <h4 class="font-ui-label text-ui-label font-bold mb-1">Visibility Settings</h4>
                    <p class="font-metadata text-metadata text-secondary">Your profile is <strong>public</strong> — posts and bio are visible to everyone on the internet.</p>
                </div>
            </div>
            <div class="p-5 bg-surface-container rounded-xl border border-outline-variant flex gap-4">
                <span class="material-symbols-outlined text-primary mt-0.5 flex-shrink-0" style="font-variation-settings: 'FILL' 1;">verified</span>
                <div>
                    <h4 class="font-ui-label text-ui-label font-bold mb-1">Verified Author</h4>
                    <p class="font-metadata text-metadata text-secondary">Your verified badge appears next to your username across all public feeds.</p>
                </div>
            </div>
        </div>
