    ...
    <div class="w-full max-w-[440px]">
        ...
        <div class="mb-8 flex items-center gap-4 p-4 rounded-lg bg-surface-container border border-outline-variant">
            ...
            <div>
                ...
                <p class="font-metadata text-metadata text-secondary">Your reset token is valid. You may now choose a new
                    password.</p>
            </div>
        </div>
        ...
        <div class="mb-4">
            @if (session('status'))
                <div
                    class="w-full px-4 py-3 bg-success-container text-success-container-fixed rounded-lg flex items-center gap-2 text-sm font-medium">
            ...
            @error('*')
                <div
                    class="w-full px-4 py-3 bg-error-container text-error-container-fixed rounded-lg flex items-center gap-2 text-sm font-medium">
        ...
        <div class="paper-card border border-outline-variant p-8 md:p-10 rounded-xl">
            <div class="mb-10 text-center">
                ...
                <p class="font-body-md text-body-md text-secondary">Ensure your account is secure by using a complex, unique
                    password.</p>
            </div>
            <form action="{{ route('password.update') }}" class="space-y-8" method="POST">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <input
                            ...
                            id="new_password" name="password" placeholder="••••••••" required="" type="text" />
                        <button class="absolute right-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-outline"
                ...
                <div class="space-y-2">
                    <label class="block font-ui-label text-ui-label text-on-surface-variant" for="confirm_password">Confirm
                        New Password</label>
                    <div class="relative">
                        <input
                            class="w-full px-4 py-3 bg-surface border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary-container focus:border-primary outline-none transition-all font-ui-label text-ui-label"
        ...
        <div class="mt-12 grid grid-cols-1 gap-4">
            <div class="paper-card border border-outline-variant p-6 rounded-xl flex items-center gap-6">
                ...
                <div>
                    ...
                    <p class="font-metadata text-metadata text-secondary leading-relaxed">We recommend using a password
                        manager and enabling two-factor authentication for maximum account protection.</p>
                </div>
            </div>
        </div>
    </div>
